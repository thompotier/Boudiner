...
try
{
    ...
    $result = $dbh->prepare("select * from Option_Appartement WHERE Statut=0");
	$result->execute();

    debutEntete();
   	while ($row = $result->fetch()) {
		afficherOption($row[ID], $row[ID_Utilisateur], $row[ID_Appartement],$row[Statut]);
    	//echo($row);
		//print_r($row);
  	}  	
	finEntete();
...
?>
		...
		?>
						...
						<?php
						function afficherOption($ido, $idu, $ida, $statut){
							echo'
							<article class="mini-post">
											<header>
												<h3><a href="#">Option '.$ido.' Utilisateur : '.$idu.' Appartement : '.$ida.'</a></h3>
												<time class="published" datetime="TEST">'.$datecreation.'</time>
												<a href="#" class="author"><img src="images/avatar.jpg" alt="" /></a>
											</header>';
                                            if (isset($_SESSION['droit']) && $_SESSION['droit']=='3') {
												echo'<form method="post" action="optionValider.php">
												...
												<form method="post" action="optionRefuser.php">
                                                    <input type="hidden" name="ido" value="'.$ido.'" />
													<input type="hidden" name="ida" value="'.$ida.'" />
